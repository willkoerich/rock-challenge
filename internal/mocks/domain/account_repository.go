...
// Update provides a mock function with given fields: ctx, transaction, account
func (_m *AccountRepository) Update(ctx context.Context, transaction database.Transaction, account internaldomain.Account) error {
	ret := _m.Called(ctx, transaction, account)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, database.Transaction, internaldomain.Account) error); ok {
		r0 = rf(ctx, transaction, account)
	} else {
		r0 = ret.Error(0)
	}
